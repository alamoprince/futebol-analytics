...
class CLIDashboard:
    ...
    def _init_github_manager(self) -> bool: 
        ...
        try: 
            ...
            self.github_manager = GitHubManager(); return True
        except Exception as e: console.print(f"[red]Erro GitHub Init: {e}[/]"); 
        self.github_manager = None; 
        return False
    ...
    def display_model_stats(self): 
        ...
        content.append(f"Arquivo: {os.path.basename(MODEL_SAVE_PATH)}\n"); 
        ...
        if self.model_eval_metrics:
            ...
            conf_matrix = self.model_eval_metrics.get('confusion_matrix'); 
            profit = self.model_eval_metrics.get('profit'); 
            roi = self.model_eval_metrics.get('roi'); 
            n_bets = self.model_eval_metrics.get('num_bets'); 
            train_n = self.model_eval_metrics.get('train_set_size', 'N/A'); 
            test_n = self.model_eval_metrics.get('test_set_size', 'N/A')
    ...
    def load_existing_model_assets(self):
        ...
        if load_result:
            ...
            if model and features: 
                ...
                console.print(f"[green]Modelo {MODEL_TYPE_NAME} e stats carregados.[/]"); 
                model_loaded_success = True; self.display_model_stats()
    ...
    def train_model(self):
        ...
        try:
            ...
            _, X_test_full, _, _ = train_test_split(df_hist_aligned, 
                                                    y_processed, 
                                                    test_size=TEST_SIZE, 
    ...
    def predict_fixtures(self): 
        ...
        try:
            ...
            if df_predictions is not None and not df_predictions.empty:
                console.print("\n[bold]--- Previsões ---[/]"); 
                table = Table(show_header=True, header_style="bold magenta", title="Probs: Ñ Emp vs Emp")
                
                # Adiciona colunas de odds e probabilidade
                display_cols_base = ['Date_Str', 'Time_Str', 'League', 'HomeTeam', 'AwayTeam']; 
                odds_cols_display_names = [ODDS_COLS['home'], ODDS_COLS['draw'], ODDS_COLS['away'], 'Odd_Over25_FT', 'Odd_BTTS_Yes']; 
                odds_cols_display = [c for c in odds_cols_display_names if c in df_predictions.columns]; 
                prob_cols_display = [c for c in df_predictions.columns if c.startswith('Prob_')]; 
                valid_cols_cli = display_cols_base + odds_cols_display + prob_cols_display; 
                valid_cols_cli = [c for c in valid_cols_cli if c in df_predictions.columns]
                for col in valid_cols_cli: header=col.replace('Date_Str','Data').replace('Time_Str','Hora').replace('HomeTeam','Casa').replace('AwayTeam','Fora'); 
                ...
                style = "dim" if col in ['Date_Str', 'Time_Str'] else ("cyan" if col.startswith('Odd_') else ("yellow" if col=='Prob_Nao_Empate' else ("bold green" if col=='Prob_Empate' else ""))); 
                ...
                if save_to_github and self._init_github_manager() and self.github_manager: # ... (código de salvar como V11) ...
                     ...
                     fetch_date_str = fetch_date.strftime("%Y-%m-%d"); filename = f"previsoes_{MODEL_TYPE_NAME}_{fetch_date_str}.csv"; 
                     github_path = f"{GITHUB_PREDICTIONS_PATH.strip('/')}/{filename}"; 
                     console.print(f"Salvando em [cyan]{GITHUB_REPO_NAME}/{github_path}[/]..."); 
                     success = self.github_manager.save_or_update_github(df=df_predictions, path=github_path, commit_message_prefix=f"Previsões {MODEL_TYPE_NAME} {fetch_date_str}: ")
                     if success:
                         console.print(f"[green]Previsões salvas com sucesso em {github_path}.[/]")
                     else:
                         console.print(f"[red]Falha ao salvar previsões em {github_path}.[/]")
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == "1":
                self.train_model() # Atualiza self.trained_model internamente
            elif choice == "2":
                if self.trained_model: # Checa ANTES de chamar
                    self.predict_fixtures()
                else:
                    console.print("[bold yellow]Nenhum modelo treinado disponível. Treine (Opção 1).[/]")
            elif choice == "3": self.display_model_stats()
