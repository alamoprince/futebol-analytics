...
from config import (
    ...
    FIXTURE_CSV_COL_MAP, REQUIRED_FIXTURE_COLS, TARGET_LEAGUES
)
...
def load_historical_data(file_path: str = HISTORICAL_DATA_PATH) -> Optional[pd.DataFrame]:
    ...
    try:
        df = pd.read_excel(file_path); print(f"Histórico carregado: {file_path} (Shape: {df.shape})")
        required_excel_names = list(set(EXCEL_EXPECTED_COLS)) # Usa set p/ evitar duplicação
        ...
        if missing_extras: print(f"Aviso: Colunas odds extras ({missing_extras}) não encontradas no histórico.")
        if missing_minimum: print(f"Erro CRÍTICO: Colunas mínimas faltando no Histórico: {missing_minimum}"); return None
...
def preprocess_and_feature_engineer(df_loaded: pd.DataFrame) -> Optional[Tuple[pd.DataFrame, pd.Series, List[str]]]:
    ...
    # Chama funções públicas refatoradas
...
def prepare_fixture_data(fixture_df: pd.DataFrame, historical_df: pd.DataFrame, feature_columns: List[str]) -> Optional[pd.DataFrame]:
    ...
    # 1. Processar Histórico (Garante Ptos, VG/CG raw)
    ...
    # 3. Juntar Médias e Calcular Derivadas (usando função pública)
    print("  Calculando features derivadas (CV_HDA, Diff_Media_CG)...")
    df_temp = fixture_df.join(df_rolling_features)
    df_with_derived = calculate_derived_features(df_temp) # Chama função PÚBLICA

    # 4. Selecionar Features Finais e Tratar NaNs
    print("  Selecionando features finais e tratando NaNs..."); 
