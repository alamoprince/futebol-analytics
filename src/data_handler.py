...
#--- PREPARAR DADOS FUTUROS (BackDraw) ---
def prepare_fixture_data(fixture_df: pd.DataFrame, historical_df: pd.DataFrame, feature_columns: List[str]) -> Optional[pd.DataFrame]:
    ...
    print("  Processando histórico p/ cálculo de médias...")
    start_time = time.time()
    historical_df_processed = calculate_historical_intermediate(historical_df.copy())
    required_hist_cols = {'Home', 'Away', 'Date', 'Ptos_H', 'Ptos_A', 'VG_H_raw', 'VG_A_raw', 'CG_H_raw', 'CG_A_raw'}
    missing_hist = [c for c in required_hist_cols if c not in historical_df_processed.columns]
    if missing_hist:
        print(f"Erro: Colunas intermediárias ausentes no histórico: {missing_hist}")
        return None
    ...
    # --- Loop para calcular médias (como antes) ---
        ...
        for team_perspective, team_name in [('H', home_team), ('A', away_team)]:
            for stat_prefix in stats_to_roll:
                ...
                if team_name and team_name in teams_in_hist:
                    team_hist = historical_df_processed[
                        (historical_df_processed['Home'] == team_name) | (historical_df_processed['Away'] == team_name)
                    ].head(ROLLING_WINDOW)
                    if not team_hist.empty:  # Garante que há histórico para o time
                        if stat_prefix == 'Ptos':
                            ...
                            continue
                        # Aplica a lógica para extrair valor H ou A baseado na perspectiva
                        stat_vals = team_hist.apply(
                            lambda r: r.get(base_h) if r['Home'] == team_name else r.get(base_a), axis=1
                        ).dropna().tolist()
                        if stat_vals:
                            media_val = np.mean(stat_vals)
                match_rolling[f'Media_{stat_prefix}_{team_perspective}'] = media_val
        rolling_features_list.append(match_rolling)

    if not rolling_features_list:  # Segurança extra
        print("Erro: Nenhuma feature rolling calculada.")
        return None
    df_rolling_features = pd.DataFrame(rolling_features_list).set_index('Index')
    print(f"  Médias Rolling calculadas. Colunas: {list(df_rolling_features.columns)}")  # DEBUG

    # 3. Juntar Médias Rolling ao fixture_df original (que tem as odds)
    # Usa left join para manter todos os jogos do fixture_df original
    df_temp = fixture_df.join(df_rolling_features, how='left')
    print(f"  Shape após juntar médias rolling: {df_temp.shape}")  # DEBUG
    print(f"  Colunas após juntar médias rolling: {list(df_temp.columns)}")  # DEBUG

    # 4. Calcular Features Derivadas (CV_HDA, Diff_Media_CG) a partir do df_temp
    print("  Calculando features derivadas (CV_HDA, Diff_Media_CG)...")
    df_with_derived = calculate_derived_features(df_temp)  # Calcula no DF já com as médias
    print(f"  Colunas após calcular derivadas: {list(df_with_derived.columns)}")  # DEBUG

    # 5. Selecionar APENAS as FEATURE_COLUMNS finais
    print("  Selecionando features finais e tratando NaNs...")
    print(f"    Features esperadas (config): {feature_columns}")  # DEBUG

    # Garante que colunas que vêm direto do CSV/Histórico (e são features) existam
    direct_features = ['Odd_H_FT', 'Odd_D_FT', 'Odd_Over25_FT', 'Odd_BTTS_Yes']
    for f in direct_features:
        if f not in df_with_derived.columns:
            print(f"    AVISO: Feature direta '{f}' ausente. Adicionando com NaN.")
            df_with_derived[f] = np.nan

    # Verifica quais features FINAIS estão faltando AGORA
    missing_final = [f for f in feature_columns if f not in df_with_derived.columns]
    if missing_final:
        print(f"    AVISO: Colunas FINAIS ausentes após todos os cálculos: {missing_final}. Serão NaN.")
        for col in missing_final:
            df_with_derived[col] = np.nan

    # Seleciona APENAS as colunas FINAIS na ordem correta
    # Garante que só seleciona colunas que existem para evitar KeyError
    cols_to_select = [f for f in feature_columns if f in df_with_derived.columns]
    print(f"    Colunas que serão selecionadas: {cols_to_select}")  # DEBUG
    X_fixture_prepared = df_with_derived[cols_to_select].copy()

    # Adiciona colunas faltantes se a seleção falhou (como fallback, preenchidas com 0 depois)
    if X_fixture_prepared.shape[1] != len(feature_columns):
        print(f"    ALERTA: Shape incorreto ({X_fixture_prepared.shape[1]}) vs esperado ({len(feature_columns)}). Adicionando colunas faltantes com NaN.")
        ...
        X_fixture_prepared = X_fixture_prepared[feature_columns]  # Reordena

    print(f"    Shape APÓS seleção final: {X_fixture_prepared.shape}")  # DEBUG

    # Tratar NaNs finais
    nan_counts = X_fixture_prepared.isnull().sum()
    if nan_counts.sum() > 0:
        print(f"Aviso: {nan_counts.sum()} NaNs finais. Preenchendo com 0.")
        X_fixture_prepared = X_fixture_prepared.fillna(0)
    print(f"Features finais preparadas. Shape: {X_fixture_prepared.shape}")
