...
from collections import defaultdict # Para cache PDP
...
from sklearn.inspection import PartialDependenceDisplay, permutation_importance, partial_dependence # Importa função base PDP
...
try:
    from config import (RANDOM_STATE, BEST_F1_MODEL_SAVE_PATH, BEST_ROI_MODEL_SAVE_PATH,
                        MODEL_ID_F1, MODEL_ID_ROI, FEATURE_EPSILON) # Importa epsilon se necessário
...
# --- Constantes e Diretórios de Cache ---
...
class ModelInterpreterApp:
    ...
    def __init__(self, parent_frame, main_root, analyzer_app_ref):
        ...
        self.model_importance_text=None; self.status_label=None; self.model_selector_var=tk.StringVar(); self.model_selector_combo=None;
        self.pdp_feature_selector_var=tk.StringVar(); self.pdp_feature_selector_combo=None; self.load_trained_models_button=None;
        self.shap_summary_canvas_widget=None; self.pdp_canvas_widget=None; self.fig_shap_summary=None; self.ax_shap=None;
        self.fig_pdp=None; self.ax_pdp=None; self.shap_summary_container=None; self.pdp_container=None;
    ...
    def create_widgets(self):
        """Cria widgets para interpretação do modelo."""
        ...
        self.load_trained_models_button=ttk.Button(model_controls_frame,text="Carregar Modelos e Dados p/ Análise",command=self.load_models_and_data); 
        self.load_trained_models_button.pack(side=tk.LEFT,padx=(0,10)); 
        ttk.Label(model_controls_frame,text="Analisar Modelo:").pack(side=tk.LEFT,padx=(10,5)); 
        self.model_selector_combo=ttk.Combobox(model_controls_frame,textvariable=self.model_selector_var,state="disabled",width=25); 
        self.model_selector_combo.pack(side=tk.LEFT); 
        self.model_selector_combo.bind("<<ComboboxSelected>>",self.on_model_select_change); 
        self.status_label=ttk.Label(model_controls_frame,text="Carregue modelos e dados..."); 
        self.status_label.pack(side=tk.LEFT,padx=10,fill=tk.X,expand=True); 
        importance_frame=ttk.LabelFrame(main_content_frame,text=" Importância de Features (Modelo Selecionado) ",padding=10); 
        importance_frame.pack(fill=tk.X,pady=5,padx=10); self.model_importance_text=self._create_scrolled_text(importance_frame,height=15); 
        self._update_text_widget(self.model_importance_text,"Carregue modelos e dados...");
        model_plots_frame=ttk.Frame(main_content_frame); 
        model_plots_frame.pack(fill=tk.BOTH,expand=True,pady=(10,0),padx=10); 
        plots_center_frame = ttk.Frame(model_plots_frame)
        plots_center_frame.pack(fill=tk.X, expand=False, pady=5)
        self.shap_summary_container=ttk.LabelFrame(model_plots_frame,text=" SHAP Summary Plot ",padding=7); 
        self.shap_summary_container.pack(fill=tk.X,expand=True,padx=7,pady=(0,7)); 
        ttk.Label(self.shap_summary_container,text="Selecione modelo...").pack();
        if not SHAP_AVAILABLE: ttk.Label(self.shap_summary_container,text="(Biblioteca 'shap' não instalada)",foreground="red").pack();
        pdp_outer_container=ttk.LabelFrame(model_plots_frame,text=" Partial Dependence Plot (PDP) ",padding=7); 
        pdp_outer_container.pack(fill=tk.BOTH,expand=True,padx=7,pady=(7,0)); 
        pdp_controls_frame=ttk.Frame(pdp_outer_container); 
        pdp_controls_frame.pack(fill=tk.X,pady=(0,5)); 
        ttk.Label(pdp_controls_frame,text="Feature para PDP:").pack(side=tk.LEFT,padx=(0,5)); 
        self.pdp_feature_selector_combo=ttk.Combobox(pdp_controls_frame,textvariable=self.pdp_feature_selector_var,state="disabled",width=25); 
    ...
    def _clear_matplotlib_widget(self, container_widget):#...idêntico...
        if container_widget:
            for widget in container_widget.winfo_children(): widget.destroy()
    def _embed_matplotlib_figure(self, fig: plt.Figure, container_widget: ttk.Frame):# ... (lógica reutilização canvas) ...
        ...
        else: logger.warning(f"Container desconhecido embed: {container_widget}")
        ...
        if existing_canvas and isinstance(existing_canvas, FigureCanvasTkAgg) and plt.fignum_exists(fig.number):
             logger.debug(f"Reutilizando canvas {canvas_attr_name}"); existing_canvas.figure = fig; existing_canvas.draw_idle(); return existing_canvas
        else:
             logger.debug(f"Criando novo canvas {canvas_attr_name or 'desconhecido'}")
             self._clear_matplotlib_widget(container_widget);
             try:
                 canvas = FigureCanvasTkAgg(fig, master=container_widget); canvas.draw(); widget = canvas.get_tk_widget(); widget.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
    ...
    def log(self, message: str):#...idêntico...
    ...
    def load_models_and_data(self):
        """Carrega modelos, dados da Aba 3 e prepara X_prepared."""
        self.log("Carregando modelos e dados p/ análise...")
        # ... (limpeza inicial como antes) ...
        self.loaded_trained_models = {}; self.model_selector_var.set(""); self.pdp_feature_selector_var.set("")
        self.X_analysis_data = None; self.current_data_identifier = None
        try: self.model_selector_combo.config(values=[], state="disabled"); self.pdp_feature_selector_combo.config(values=[], state="disabled")
        except: pass
        self._update_text_widget(self.model_importance_text,"Carregando..."); self._clear_matplotlib_widget(self.shap_summary_container); self._clear_matplotlib_widget(self.pdp_container)
        ttk.Label(self.shap_summary_container, text="Carregando...").pack(); ttk.Label(self.pdp_container, text="Carregando...").pack(); self.parent.update_idletasks()

        # 1. Pega dados da Aba Análise
        if not hasattr(self.analyzer_app,'X_clean') or self.analyzer_app.X_clean is None or \
           not hasattr(self.analyzer_app,'current_data_identifier') or self.analyzer_app.current_data_identifier is None:
            messagebox.showerror("Erro Dados","Carregue dados na Aba 'Análise Features' primeiro.",parent=self.parent); self.log("Erro: Dados Aba Análise ausentes."); return
        else:
            self.X_analysis_data = self.analyzer_app.X_clean.copy(); self.current_data_identifier = self.analyzer_app.current_data_identifier
            self.log(f"Dados ({self.X_analysis_data.shape}) obtidos. ID: {self.current_data_identifier}")

        # 2. Carrega Modelos, Prepara Dados, Carrega Cache
        ...
        for model_id, model_path in model_paths_to_try.items():
            ...
            load_result = load_model_scaler_features(model_path) # Esta função retorna scaler também
            if load_result:
                model, scaler, calib, ev_thr, f1_thr, features, params, metrics, ts = load_result
                if model and features:
                    if not all(f in self.X_analysis_data.columns for f in features): self.log(f"    Aviso: Features {model_id} incompatíveis."); continue

                    # Prepara X_prepared (subset + scaling se necessário)
                    X_prepared_for_model = None; X_model_subset_unscaled = self.X_analysis_data[features].copy()
                    if scaler:
                        self.log(f"    -> Aplicando scaler {model_id}...");
                        try: 
                            X_prep_temp=X_model_subset_unscaled.replace([np.inf,-np.inf],np.nan); X_prep_temp.fillna(X_prep_temp.median(),inplace=True);
                            if X_prep_temp.isnull().values.any(): raise ValueError("NaNs pós-imputação")
                            X_scaled_np=scaler.transform(X_prep_temp); X_prepared_for_model=pd.DataFrame(X_scaled_np,index=X_prep_temp.index,columns=features)
                        except Exception as e: logger.error(f"Erro scaler {model_id}: {e}"); self.log(f"    -> Erro scaler {model_id}.")
                    else: X_prepared_for_model = X_model_subset_unscaled
                    if X_prepared_for_model is None: self.log(f"    -> Falha preparar dados {model_id}."); continue

                    # Calcula importância usando X_prepared
                    imp_df = self._calculate_feature_importance(model, features, X_prepared_for_model)
                    if imp_df is None: logger.warning(f"Não calculou importância {model_id}.")

                    # Armazena tudo no dicionário
                    model_entry = {'model':model,'scaler':scaler,'calibrator':calib,'optimal_ev_threshold':ev_thr,'optimal_f1_threshold':f1_thr,'features':features,'params':params,'metrics':metrics,'timestamp':ts,'path':model_path,'importances':imp_df,'X_prepared':X_prepared_for_model}
                    self.loaded_trained_models[model_id] = model_entry; loaded_ids.append(model_id); all_feature_names.update(features); self.log(f"    -> Modelo {model_id} e dados prep OK.")

                    # Tenta carregar cache SHAP (validando ID dados)
                    ...
                    if os.path.exists(shap_cache_path):
                        try: 
                            ...
                            if isinstance(shap_cache_data,dict) and shap_cache_data.get('shap_data_identifier')==self.current_data_identifier and shap_cache_data.get('shap_model_features')==features: self.loaded_trained_models[model_id].update(shap_cache_data); self.log(f"    -> Cache SHAP validado.")
                            else: 
                                self.log(f"    -> Cache SHAP inválido.")
                        except Exception as e: logger.error(f"Erro carregar cache SHAP: {e}"); self.log(f"    -> Erro cache SHAP.")
                    else: self.log(f"    -> Cache SHAP não encontrado.")
                else: self.log(f"    -> Falha (modelo/features ausentes).")
            else: self.log(f"    -> Falha geral ao carregar {model_id}.")

        # Atualiza GUI
        if loaded_ids: # ... (atualiza comboboxes como antes) ...
        ...
        else: # ... (limpa GUI como antes) ...
             self.log("Nenhum modelo válido."); # ... (resto limpeza) ...

    # --- Helpers para nomes de cache ---
    def _get_cache_path(self, model_id: str, cache_type: str, cache_dir: str) -> str: # Helper genérico
        safe_model_id = model_id.replace(" ","_").replace("(","").replace(")","").lower(); data_hash="no_data"
        if self.current_data_identifier:
            try: id_str = str(self.current_data_identifier); data_hash = hashlib.sha1(id_str.encode()).hexdigest()[:8]
            except Exception: data_hash="error_hash"
        filename = f"{cache_type}_cache_{safe_model_id}_data_{data_hash}.joblib"; return os.path.join(cache_dir,filename)
    def _get_shap_cache_path(self, model_id: str) -> str: return self._get_cache_path(model_id, "shap", SHAP_CACHE_DIR)
    # REMOVIDO: _get_pdp_cache_path
    ...
    def _calculate_feature_importance(self, model, model_features: List[str], X_prepared_for_model: pd.DataFrame) -> Optional[pd.DataFrame]:
        # ... (código idêntico, usa X_prepared_for_model) ...
        importances = None; model_type_name = model.__class__.__name__; logger.debug(f"Calculando importância para {model_type_name}...")
        if not hasattr(self.analyzer_app, 'y_clean') or self.analyzer_app.y_clean is None: logger.error("y_clean não disponível."); return None
        try: y_aligned = self.analyzer_app.y_clean.loc[X_prepared_for_model.index].copy()
        except Exception as e: logger.error(f"Erro alinhar y importância: {e}"); return None
        try:
            ...
            else: logger.warning(f"Não obteve importâncias {model_type_name}."); return None
    ...
    def on_model_select_change(self, event=None):
        """Chamado quando o modelo é trocado."""
        logger.debug(f"Model selection changed: {self.model_selector_var.get()}")
        self.update_model_specific_analysis()

    def update_model_specific_analysis(self):
        """Atualiza painéis com base no modelo selecionado, dispara SHAP/PDP."""
        selected_id = self.model_selector_var.get()
        if not selected_id or selected_id not in self.loaded_trained_models: # ... (limpa e retorna) ...
             self._update_text_widget(self.model_importance_text,"Selecione."); self._clear_matplotlib_widget(self.shap_summary_container); ttk.Label(self.shap_summary_container,"...").pack(); self._clear_matplotlib_widget(self.pdp_container); ttk.Label(self.pdp_container,"...").pack();
        ...
        if importances_df is not None and not importances_df.empty: #... (exibe ou loga erro)
             try: self._update_text_widget(self.model_importance_text, importances_df.round(5).to_string(index=True))
             except Exception as e: logger.error(f"Erro formatar imp: {e}"); self._update_text_widget(self.model_importance_text, "Erro.")
        else: self._update_text_widget(self.model_importance_text, f"Importância não disponível {selected_id}.")
        # Inicia SHAP
        self._start_shap_calculation(model_data)
        # Atualiza PDP (popula e dispara via debounce)
        pdp_features = model_data.get('features', [])
        current_pdp_list_str = str(self.pdp_feature_selector_combo.cget('values')); new_pdp_list_str = str(tuple(pdp_features))
        if pdp_features and current_pdp_list_str != new_pdp_list_str :
             logger.debug("Atualizando lista PDP."); self.pdp_feature_selector_combo.config(values=pdp_features, state="readonly");
        ...
        elif pdp_features: logger.debug("Lista PDP OK, disparando on_pdp_feature_select."); self.on_pdp_feature_select()
    ...
    # --- Funções SHAP (com cache em memória e disco) ---
    def _start_shap_calculation(self, model_data: Dict):
        # ... (Lógica idêntica à anterior, com verificação de cache robusta) ...
        ...
        if model_data.get('X_prepared') is None: self._clear_matplotlib_widget(self.shap_summary_container); ttk.Label(self.shap_summary_container, text="Dados preparados ausentes.").pack(); return
        ...
        if 'shap_values' in model_data and 'shap_data_identifier' in model_data:
            ...
            if cached_data_id == self.current_data_identifier and cached_features == features_for_model: self.log(f"Reutilizando SHAP cacheado (memória) {model_id}."); use_cache = True
            else: 
                self.log(f"Cache SHAP memória {model_id} invalidado."); # ... (limpa chaves) ...
        ...
        if use_cache: shap_payload_cached={'model_id':model_id,'shap_values':model_data['shap_values'],'X_sample':model_data['X_sample'],'model_type_name':model.__class__.__name__,'used_kernel':model_data.get('shap_used_kernel',False),'n_samples':model_data.get('shap_n_samples','?')}; self._plot_shap_summary(shap_payload_cached); return
        ...
        if use_kernel_explainer: # Roda em background
            self.log(f"Iniciando SHAP {model_type_name} background..."); self._clear_matplotlib_widget(self.shap_summary_container); ttk.Label(self.shap_summary_container, text=f"Calculando SHAP {model_type_name}...").pack()
    ...
    def _run_shap_task(self, target_model_id: str, use_kernel_explainer: bool):
        # ... (Lógica idêntica à anterior, usando 'X_prepared') ...
        shap_values_for_plot = None; X_shap_sample = None;
        if target_model_id not in self.loaded_trained_models: logger.error(f"Thread SHAP: Modelo {target_model_id} não carregado."); self.gui_queue.put(("shap_error", {'model_id': target_model_id, 'error': "Dados modelo."})); return
        ...
        try:
            ...
            if use_kernel_explainer: #... (kernel explainer logic) ...
                 ...
                 explainer=shap.KernelExplainer(model_predict_proba_pos,X_kernel_bg); shap_values=explainer.shap_values(X_shap_sample, silent=True) # Adiciona silent
                 ...
                 explainer=shap.KernelExplainer(model_predict_proba_pos_fb, X_kernel_bg); shap_values=explainer.shap_values(X_shap_sample, silent=True) # Adiciona silent
            # ... (Processamento shap_values -> shap_values_for_plot) ...
            ...
            if shap_values_for_plot is not None and X_shap_sample is not None:
                shap_cache_data = {'shap_values': shap_values_for_plot, 'X_sample': X_shap_sample, 'shap_data_identifier': self.current_data_identifier, 'shap_model_features': features, 'shap_used_kernel': use_kernel_explainer, 'shap_n_samples': N_SHAP_SAMPLES}
                ...
                if target_model_id in self.loaded_trained_models: # Atualiza memória
                    try: self.loaded_trained_models[target_model_id].update(shap_cache_data); cache_updated_mem = 'shap_values' in self.loaded_trained_models[target_model_id]; logger.info(f"Cache SHAP memória {target_model_id} OK.")
    ...
    # --- Funções de Plotagem (SHAP/PDP - Reutilizam Eixos/Canvas) ---
    def _plot_shap_summary(self, shap_data: Dict):
        """Gera gráfico SHAP, salva como imagem e exibe, desabilitando tight_layout."""
        model_id_for_plot = shap_data.get('model_id', 'Desconhecido')
        self.log(f"Gerando gráfico SHAP para {model_id_for_plot} (salvando imagem, suprimindo tight)...")
        self._clear_matplotlib_widget(self.shap_summary_container)

        fig_temp = None
        # Usa um nome de arquivo temporário mais simples dentro do cache dir
        temp_image_path = os.path.join(SHAP_CACHE_DIR, f"temp_shap_summary_{os.getpid()}.png") # Adiciona PID para evitar conflito de threads

        # --- Salva estado original do rcParams ---
        original_autolayout = plt.rcParams['figure.autolayout']
        original_constrained = plt.rcParams.get('figure.constrained_layout.use', False) # get com default
        ...
        try:
            ...
            if not feature_names_plot: raise ValueError("Nomes features ausentes SHAP plot.")

            # --- DESABILITA AUTOLAYOUT TEMPORARIAMENTE ---
            plt.rcParams['figure.autolayout'] = False
            plt.rcParams['figure.constrained_layout.use'] = False
            # --------------------------------------------

            # --- Cria figura e plota SHAP (SEM MOSTRAR) ---
            # Cria a figura ANTES de chamar shap plot
            fig_temp = plt.figure(figsize=(8, 6)) # Pode ajustar
            ...
            # SHAP plotou na figura atual (fig_temp)
            # --- FIM PLOTAGEM ---

            # --- Reconfigura título (opcional, SHAP pode ter setado) ---
            ax = fig_temp.gca()
            ...
            logger.debug(f"Salvando SHAP plot em: {temp_image_path}")
            fig_temp.savefig(temp_image_path, bbox_inches='tight', dpi=96)
            # --------------------------

        except Exception as e_plot_save:
            logger.error(f"Erro GERAL ao plotar/salvar SHAP para {model_id_for_plot}: {e_plot_save}", exc_info=True)
            ttk.Label(self.shap_summary_container, text=f"Erro ao gerar/salvar SHAP:\n{e_plot_save}").pack()
            # Não tenta carregar a imagem se salvar falhou
            temp_image_path = None # Indica que não há imagem para carregar
        finally:
            # --- SEMPRE FECHA A FIGURA E RESTAURA rcParams ---
        ...
        # --- Carrega e Exibe a Imagem Salva no Tkinter ---
        self._clear_matplotlib_widget(self.shap_summary_container) # Limpa container de novo
        if temp_image_path and os.path.exists(temp_image_path):
            try:
                img = Image.open(temp_image_path)
                photo = ImageTk.PhotoImage(img)
                img_label = ttk.Label(self.shap_summary_container, image=photo)
                img_label.image = photo
                img_label.pack(fill=tk.BOTH, expand=True)
                self.log(f"Gráfico SHAP exibido como imagem para {model_id_for_plot}.")
                # Tenta remover o arquivo temporário após exibição (opcional)
                try: os.remove(temp_image_path)
                except OSError: logger.warning(f"Não foi possível remover imagem SHAP temp: {temp_image_path}")
            except Exception as e_img:
                logger.error(f"Erro ao carregar/exibir imagem SHAP {temp_image_path}: {e_img}", exc_info=True)
                ttk.Label(self.shap_summary_container, text=f"Erro exibir imagem SHAP:\n{e_img}").pack()
        elif temp_image_path: # Se o path foi definido mas o arquivo não existe (erro no savefig?)
            logger.error(f"Imagem SHAP temporária não encontrada após tentativa de salvar: {temp_image_path}")
            ttk.Label(self.shap_summary_container, text="Erro: Imagem SHAP não gerada/encontrada.").pack()
    ...
    def on_pdp_feature_select(self, event=None):
        ...
        if self.pdp_update_job:
            ...
            except: pass # Ignora erros
        # Chama diretamente _plot_pdp após delay
        self.pdp_update_job = self.main_tk_root.after(300, self._plot_pdp) # Chama plot direto

    # --- MODIFICADO: _plot_pdp (Calcula e plota diretamente) ---
    def _plot_pdp(self):
        """Calcula e plota o PDP para a feature selecionada."""
        self.pdp_update_job = None # Reseta job debounce
        selected_model_id = self.model_selector_var.get()
        selected_pdp_feature = self.pdp_feature_selector_var.get()
        logger.debug(f"--- _plot_pdp INICIADO para {selected_model_id} - {selected_pdp_feature} ---")
        ...
        model = model_data.get('model'); features = model_data.get('features'); X_prepared = model_data.get('X_prepared')
        if not all([model, features, X_prepared is not None]) or selected_pdp_feature not in features:
             self._clear_matplotlib_widget(self.pdp_container); ttk.Label(self.pdp_container, text="Modelo/Dados/Feature inválidos.").pack(); return

        # Mostra status "Calculando"
        self._clear_matplotlib_widget(self.pdp_container)
        ttk.Label(self.pdp_container, text=f"Calculando PDP para\n{selected_pdp_feature}...").pack()
        self.parent.update_idletasks() # Força atualização

        try:
            logger.debug("Forçando recriação de figura/canvas PDP para teste.")
            # --- Reutilização/Criação da Figura/Eixo ---
            if self.fig_pdp is None or not plt.fignum_exists(self.fig_pdp.number):
                logger.debug("    Criando nova figura/eixo PDP.")
            ...
            if not self.pdp_canvas_widget: # Verifica se o embed falhou
             raise RuntimeError("Falha ao criar/embutir novo canvas PDP.")
            else:
            ...
            if len(X_prepared) > N_PDP_SAMPLES:
                 logger.debug(f"    Usando {N_PDP_SAMPLES} amostras para PDP Display.");
                 X_pdp_sample = X_prepared.sample(n=N_PDP_SAMPLES, random_state=RANDOM_STATE)
            else:
                 X_pdp_sample = X_prepared

            # --- Calcula e Plota usando PartialDependenceDisplay ---
            logger.debug(f"    Gerando PDP Display para {selected_pdp_feature}...")
            start_pdp_disp = time.time()
            try: # Tenta com target=1
                 display = PartialDependenceDisplay.from_estimator(estimator=model, X=X_pdp_sample, features=[selected_pdp_feature], kind='average', target=1, ax=self.ax_pdp, line_kw={"color": "darkcyan", "linewidth": 2.5})
            except TypeError as te: # Fallback sem target
                 if 'target' in str(te): logger.warning(f"    TypeError 'target' PDP Display. Tentando sem."); display = PartialDependenceDisplay.from_estimator(estimator=model, X=X_pdp_sample, features=[selected_pdp_feature], kind='average', ax=self.ax_pdp, line_kw={"color": "sienna", "linewidth": 2.5})
                 else: raise te
            logger.debug(f"    PartialDependenceDisplay concluído ({(time.time() - start_pdp_disp):.2f}s).")

            # Configura eixos e título
            self.ax_pdp.set_title(f'Partial Dependence Plot\n({selected_pdp_feature} vs Prob Empate)'); self.ax_pdp.set_ylabel('Dependência Parcial'); self.ax_pdp.grid(True, linestyle='--', alpha=0.6);
            # Adiciona Rug plot manualmente (opcional)
            try: sns.rugplot(x=X_pdp_sample[selected_pdp_feature], ax=self.ax_pdp, height=-0.03, clip_on=False, color='black', alpha=0.4)
            except Exception as e_rug: logger.warning(f"Erro ao adicionar rug plot: {e_rug}")

            # Redesenha o canvas
            if self.pdp_canvas_widget: self.pdp_canvas_widget.draw_idle()
            logger.debug(f"--- _plot_pdp FINALIZADO com sucesso para {selected_pdp_feature} ---")

        except Exception as e: logger.error(f"Erro GERAL ao plotar PDP para {selected_pdp_feature}: {e}", exc_info=True); self._clear_matplotlib_widget(self.pdp_container); ttk.Label(self.pdp_container, text=f"Erro plotar PDP:\n{e}").pack()

    # --- process_gui_queue (SIMPLIFICADO - sem mensagens PDP) ---
    def process_gui_queue(self):
        ...
        try:
            while True:
                ...
                try:
                    logger.debug(f"Processando msg GUI: {message_type}")
                    if message_type == "shap_result_calculated": # SHAP OK
                        ...
                        if finished_model_id == current_model_id and finished_model_id in self.loaded_trained_models:
                            ...
                            if 'shap_values' in model_data and 'X_sample' in model_data:
                                shap_plot_payload = { # Recria payload
                                    'model_id': finished_model_id, 'shap_values': model_data['shap_values'],
                                    'X_sample': model_data['X_sample'], 'model_type_name': model_data['model'].__class__.__name__,
                                    'used_kernel': model_data.get('shap_used_kernel', False), 'n_samples': model_data.get('shap_n_samples', '?')
                                }
                                self._plot_shap_summary(shap_plot_payload)
                            else: logger.error(f"Erro Plot SHAP: Cache inválido {finished_model_id}."); # ... (erro cache)
                        else: self.log(f"Ignorando SHAP obsoleto {finished_model_id}.")
                    elif message_type == "shap_error": # SHAP Erro
                         error_model_id = payload.get('model_id', 'D'); error_msg = payload.get('error', 'E'); self.log(f"Erro cálculo SHAP {error_model_id}: {error_msg}")
        ...
        finally: # ... (reagendamento como antes) ...
